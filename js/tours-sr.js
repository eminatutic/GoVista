...
async function loadDestinations() {
  ...
  if (!selectedCountry) {
    container.innerHTML = "<p>Molimo vas da prvo izaberete državu.</p>";
    return;
  }
  ...
  try {
    ...
    if (filtered.length === 0) {
      container.innerHTML = "<p>Nema pronađenih destinacija za ovu državu.</p>";
      return;
    }
    ...
    console.error(err);
    container.innerHTML = "<p>Podaci nisu mogli biti učitani.</p>";
  }
}
  ...
  return `
    <div class="destination-card">
      <div class="image-slider">
        ${dest.images?.map((img, i) => `<img src="${img}" class="${i === 0 ? "active" : ""}" alt="${dest.name}">`).join("") || "<p>Slike nisu dostupne</p>"}
